...
% Finds locally maximal or minimal values in the y direction of the 
% given data.
%
% Input:
...
%   scale       Specifies the number of data points in a local 
%               neighborhood (neighborhood legth is 2 * scale + 1)
...
% even within the min/max detection below.  For reasons opaque to me, 
% the max() function doesn't seem to work in a vector operation.  We 
% wanted something like:
% winmax(ii) = max(rdataMax(ii-scale:ii+scale));
% But, this seems to generate a vector mostly of, almost like the 
% maximum is being applied cummulatively. Oh, well...this isn't too 
% slow if the data are not too big.
...
if (nargin < 4)     % regions not defined--search everywhere
    ...
    maxima(ii) = rdataMax(ii) >= threshold & ...
        rdataMax(ii) >= winmax(ii);
    minima(ii) = rdataMin(ii) >= threshold & ...
        rdataMin(ii) >= winmin(ii);
else                % only search in specified regions
    for i = 1:length(regions)
        ii = (max(1+scale, regions(i)-scale):min(size(data,1)-scale, ...
            regions(i)+scale))';
        maxima(ii) = rdataMax(ii) >= threshold & ...
            rdataMax(ii) >= winmax(ii);
        minima(ii) = rdataMin(ii) >= threshold & ...
