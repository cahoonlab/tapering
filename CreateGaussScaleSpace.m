...
% Input:
%   data        A 1D data set
%   deriv       The order of Gaussian scale space to compute (e.g. 0 is 
%               a smoothing scale space; 1 is an edge detecting scale 
%               space)
...
for i = 1:length(scales)
    ...
    % boundaries
    padData = [data(1)*ones(length(g),1); data; ...
        data(end)*ones(length(g),1)];
